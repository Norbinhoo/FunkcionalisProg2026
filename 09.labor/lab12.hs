...
import Data.Char (isDigit, isAlpha, isAlphaNum, isUpper, ord,toLower) 
...
formatText [] = []
formatText (c:cs)
  | isPunctuation c = c : ' ' : formatText (dropWhile (== ' ') cs)
  | otherwise       = c : formatText cs

main = do
    tartalom <- readFile "09.labor/szoveg.txt"
    let formazottSzoveg = formatText tartalom
    
    putStrLn "--- A formázott szöveg: ---"
    putStrLn formazottSzoveg
